...
# - Extract the survey google sheet, and engineer a `year` column from the `Timestamp` using the `extract_year_mdy` 
#       function in `pandaslib.py`. Then save as `cache/survey.csv`

# - For each unique year in the surveys: extract the cost of living for that year from the website, 
#       engineer a `year` column for that year, then save as `cache/col_{year}.csv` for example for `2024` it would be 
#       `cache/col_2024.csv`

# After you've completed this part commit your changes to git, but DO NOT PUSH.

state_to_abbreviation_lookup_table = pd.read_csv('https://docs.google.com/spreadsheets/d/14wvnQygIX1eCVo7H5B7a96W1v5VCg6Q9yeRoESF6epw/export?format=csv')

state_to_abbreviation_lookup_table.to_csv('cache/states.csv', index=False)
